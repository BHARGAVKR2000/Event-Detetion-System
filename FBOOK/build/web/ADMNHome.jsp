<%-- 
    Document   : ADMNHome
    Created on : Oct 9, 2015, 10:21:31 AM
    Author     : alaguraj
--%>
...
<table border="2" style="color: red;font-size: 10px">
    <tr><td>Post ID</td><td>Report User</td><td>Comment</td></tr>
    <%
      while(rs.next())
      {
          %>
          <tr><td><%=rs.getString("Post_ID")%></td>
          <td><%=rs.getString("Cmnt_UserMail")%></td>
          <td><%=rs.getString("Comments")%></td></tr>
          <%
...
<table border="2" style="color: red;font-size: 10px">
    ...
    <%
      while(rs2.next())
      {
          ...
          {
               sts="Prome to Addict";
              out.println("Prome to Addict");
          }
         %>
          </td>
          <td><a href="charts_page.jsp?uname=<%=rs2.getString("L_uname")%>">Graph</a></td>
...
<br>
 ...
 <%
DB db2=new DB();
ResultSet rs3=db2.Select("select L_uname,sum(diff) as diff from logdet ");
%>
<table border="2" style="color: red;font-size: 10px">
    <tr><td>USer NAme</td><td>Max time</td><td>Hours</td></tr>
    <%
